...
def catalog_models_seo_meta(make: str | None, *, total: int) -> SeoMeta:
    if make:
        title = f"Модели {make} до 160 л.с. в Беларуси — Auto160"
        description = (
            f"Модели {make} до 160 л.с. в каталоге Auto160: поколения, комплектации и объявления. "
            f"Найдено моделей: {total}."
        )
        path = f"/catalog/models?make={quote(make)}"
        h1 = f"Модели {make} до 160 л.с."
        intro = (
            f"Выберите модель {make} с двигателем до 160 л.с.: сравните поколения и "
