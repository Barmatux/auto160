#!/usr/bin/env python3
"""Import Auto24 listings from scrape Postgres into Auto160 car_listings.

Example:

  python tools/import_auto24_listings.py --dry-run --limit 20
...
def _ensure_importer_user(db) -> User:
    existing = db.query(User).filter(User.username == "auto24_importer").first()
    if existing:
        return existing
    user = User(
        username="auto24_importer",
        email="auto24_importer@auto160.local",
        name="Auto24 Importer",
...
def _apply_mapped(listing: CarListing, mapped, *, seller_id: int) -> None:
    ...
    listing.city = (mapped.city or "Эстония")[:80]
    ...
    listing.description = (
        f"{description}\n\n"
        f"Источник: auto24.ee\n"
        f"URL: {mapped.source_url or ''}\n"
        f"AUTO24_ID: {mapped.external_id}"
    ).strip()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import Auto24 listings into Auto160")
        ...
        for row in rows:
            ...
            if existing is None:
                ...
                listing = CarListing(
                    ...
                    city=(mapped.city or "Эстония")[:80],
                    description="",
