#!/usr/bin/env python3
"""Import mobile.de listings from scrape Postgres into Auto160 car_listings.

Example:

  python tools/import_mobile_de_listings.py --dry-run --limit 20
...
def _ensure_importer_user(db) -> User:
    existing = db.query(User).filter(User.username == "mobile_de_importer").first()
    if existing:
        return existing
    user = User(
        username="mobile_de_importer",
        email="mobile_de_importer@auto160.local",
        name="mobile.de Importer",
...
def _apply_mapped(listing: CarListing, mapped, *, seller_id: int) -> None:
    ...
    listing.city = (mapped.city or "Германия")[:80]
    ...
    listing.description = (
        f"{description}\n\n"
        f"Источник: mobile.de\n"
        f"URL: {mapped.source_url or ''}\n"
        f"MOBILE_DE_ID: {mapped.external_id}"
    ).strip()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import mobile.de listings into Auto160")
    ...
    try:
        ...
        existing_rows = db.query(CarListing).filter(CarListing.source == SOURCE).all()
        by_external = {row.external_id: row for row in existing_rows if row.external_id}

        for row in rows:
            mapped = map_mobile_de_row(
                dict(row),
                eur_rate=eur,
                max_hp=args.max_hp,
            ...
            if existing is None:
                ...
                listing = CarListing(
                    ...
                    city=(mapped.city or "Германия")[:80],
                    description="",
