#!/usr/bin/env python3
"""Periodically import mobile.de listings from scrape-platform Postgres into Auto160."""
...
IMPORTER_PATH = ROOT_DIR / "tools" / "import_mobile_de_listings.py"
...
def run_embedding_reindex() -> int:
    ...
    logger.info("mobile-de-embeddings-start: %s", " ".join(cmd))
    result = subprocess.run(cmd, cwd=str(ROOT_DIR), capture_output=True, text=True)
    if result.stdout:
        for line in result.stdout.splitlines()[-20:]:
            logger.info("mobile-de-embeddings | %s", line)
    if result.stderr:
        for line in result.stderr.splitlines()[-10:]:
            logger.warning("mobile-de-embeddings ! %s", line)
    if result.returncode != 0:
        logger.warning("mobile-de-embeddings failed exit_code=%s; continuing", result.returncode)
        return 0
    logger.info("mobile-de-embeddings-finish: exit_code=%s", result.returncode)
    return 0
...
def run_once(
    *,
    ...
    logger.info("mobile-de-import-start: %s", " ".join(cmd))
    result = subprocess.run(cmd, cwd=str(ROOT_DIR), capture_output=True, text=True)
    if result.stdout:
        for line in result.stdout.splitlines()[-40:]:
            logger.info("mobile-de-import | %s", line)
    if result.stderr:
        for line in result.stderr.splitlines()[-20:]:
            logger.warning("mobile-de-import ! %s", line)
    logger.info("mobile-de-import-finish: exit_code=%s", result.returncode)
    if result.returncode != 0:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Schedule mobile.de → Auto160 import")
