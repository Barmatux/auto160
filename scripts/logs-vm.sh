...
usage() {
  ...
  echo "Services: api, avby-sync, avby-archive, avby-vin-session, autoplius-sync"
}
...
if [[ "$MODE" == "api" || "$MODE" == "avby-sync" || "$MODE" == "avby-archive" || "$MODE" == "avby-vin-session" || "$MODE" == "autoplius-sync" ]]; then
  SERVICE="$MODE"
  LINES="${2:-200}"
fi
